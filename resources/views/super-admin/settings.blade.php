...
<div class="max-w-2xl space-y-6">
    ...
    <form method="POST" action="{{ route('super.settings.update') }}" enctype="multipart/form-data">
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-700">
                <h2 class="text-base font-semibold text-white flex items-center gap-2">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    Facebook OAuth
                </h2>
                <p class="text-gray-400 text-sm mt-1">Enable "Continue with Facebook" on the login page.</p>
            </div>
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">App ID</label>
                        <input type="text" name="facebook_client_id"
                               value="{{ $settings->facebook_client_id ? '••••••••' . substr($settings->facebook_client_id, -8) : '' }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="facebook_client_secret"
                               value="{{ $settings->facebook_client_secret ? '••••••••' . substr($settings->facebook_client_secret, -6) : '' }}"
                               placeholder="abc123..."
                               class="w-full rounded-lg border border-gray-600 bg-gray-700 text-gray-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500">
                        <p class="text-gray-500 text-xs mt-1">Leave blank to keep existing. Paste full value to update.</p>
                    </div>
                ...
                <div class="bg-gray-900 rounded-lg p-3 text-xs text-gray-400 space-y-1">
                    <p class="font-medium text-gray-300">Setup instructions:</p>
                    <p>1. Go to <span class="text-blue-400">developers.facebook.com</span> → My Apps → your app</p>
                    <p>2. Under Settings → Basic, copy the App ID and App Secret</p>
                    <p>3. Under Facebook Login → Settings, add this as a Valid OAuth Redirect URI: <code class="text-green-400 bg-gray-800 px-1 rounded">{{ url('/auth/facebook/callback') }}</code></p>
                </div>
                @if($settings->hasFacebook())
                <div class="flex items-center gap-2 text-green-400 text-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    Facebook OAuth is configured. The "Continue with Facebook" button is active.
                </div>
                @else
                <div class="flex items-center gap-2 text-yellow-400 text-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Facebook OAuth is not configured. The button is visible but will error if clicked.
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-700">
                <h2 class="text-base font-semibold text-white flex items-center gap-2">
                    <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.734l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    X (Twitter) OAuth
                </h2>
                <p class="text-gray-400 text-sm mt-1">Enable "Continue with X" on the login page.</p>
            </div>
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">API Key (Consumer Key)</label>
                        <input type="text" name="twitter_client_id"
                               value="{{ $settings->twitter_client_id ? '••••••••' . substr($settings->twitter_client_id, -8) : '' }}"
                               placeholder="xxxxxxxxxxxxxxxxxxxxxxxxx"
                               class="w-full rounded-lg border border-gray-600 bg-gray-700 text-gray-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500">
                        <p class="text-gray-500 text-xs mt-1">Leave blank to keep existing. Paste full value to update.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">API Secret (Consumer Secret)</label>
                        <input type="text" name="twitter_client_secret"
                               value="{{ $settings->twitter_client_secret ? '••••••••' . substr($settings->twitter_client_secret, -6) : '' }}"
                ...
                <div class="bg-gray-900 rounded-lg p-3 text-xs text-gray-400 space-y-1">
                    <p class="font-medium text-gray-300">Setup instructions:</p>
                    <p>1. Go to <span class="text-blue-400">developer.twitter.com</span> → Projects &amp; Apps → your app</p>
                    <p>2. Under Keys and Tokens, copy the API Key and API Secret</p>
                    <p>3. Under User authentication settings, enable OAuth 1.0a with Read permissions and add this callback URL: <code class="text-green-400 bg-gray-800 px-1 rounded">{{ url('/auth/twitter/callback') }}</code></p>
                    <p class="text-yellow-400">Note: X does not always provide the user's email address. New users may be prompted to enter it manually.</p>
                </div>
                @if($settings->hasTwitter())
                <div class="flex items-center gap-2 text-green-400 text-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    X OAuth is configured. The "Continue with X" button is active.
                </div>
                @else
                <div class="flex items-center gap-2 text-yellow-400 text-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    X OAuth is not configured. The button is visible but will error if clicked.
                </div>
                @endif
            </div>
        </div>
